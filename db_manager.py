...
def get_supabase_client() -> Client:
    """Crea una nueva instancia del cliente de Supabase para cada sesion."""
...
def get_user(client: Client) -> User:
    """Obtiene el perfil del usuario vinculado a su sesion de Auth."""
    try:
        ...
        new_profile = {"email": email, "nombre": email.split("@")[0], "objetivo": "Aumento de masa muscular", "peso_actual": 0.0, "genero": "Hombre", "nivel": "Novato"}
        ...
        print(f"DEBUG: Error critico en get_user: {e}")
    return None

def update_user_profile(client: Client, user_id: int, data: dict) -> bool:
    try:
        mapping = {"nombre": "nombre", "objetivo": "objetivo", "peso": "peso_actual", "genero": "genero", "nivel": "nivel", "altura": "altura", "cuello": "cuello", "cintura": "cintura", "cadera": "cadera", "pecho": "pecho", "gluteo": "gluteo", "bicep": "bicep", "muslo": "muslo", "edad": "edad", "mes_actual": "mes_actual", "entrenos_mes": "entrenos_mes"}
...
def log_workout(client: Client, user_id: int, routine_name: str) -> bool:
    try:
        ...
        mes = (total // 20) + 1
        entrenos = total % 20
        client.table("usuarios").update({"mes_actual": mes, "entrenos_mes": entrenos}).eq("id", user_id).execute()
        return True
    except Exception as e:
...
def get_last_weight(client: Client, user_id: int, exercise_name: str) -> float:
    try:
        response = client.table("records_fuerza").select("peso").eq("usuario_id", user_id).eq("ejercicio_nombre", exercise_name).order("fecha", desc=True).limit(1).execute()
        if response.data: return float(response.data[0]["peso"])
        return 0.0
    except Exception as e:
...
def get_workout_progress(client: Client, user_id: int, fecha: str) -> dict:
    try:
        ...
        print(f"Error get_workout_progress (Posible desconexion): {e}")
        return None
